      ...
      <div class="movie-info-detail clearfix">
        <div class="main-content">
          <div class="crumbs-nav-container">
            <a href="/welcome" style="color: aqua;">首页</a> &gt; <a href="/movie" style="color: aqua;">电影</a> &gt;
            {{movieInfo.movieName}}
          </div>
                ...
                <div class="module">
                  ...
                  <div class="mod-content">
                    <div class="pictures-list">
                      ...
                      <el-image class="default-img" :src="movieInfo.moviePictures[0]"
                        :preview-src-list="movieInfo.moviePictures" v-if="movieInfo.moviePictures.length > 0">
                      </el-image>
                      <div class="little-pictures">
                        <el-image class="default-img" v-for="(item,index) in movieInfo.moviePictures.slice(1)"
                          :key="index" :src="item" :preview-src-list="movieInfo.moviePictures">
                        </el-image>
                      </div>
                    </div>
                  </div>
              ...
              <div class="tab-body">
                <div class="pictures-list">
                  <div class="little-pictures">
                    ...
                    <el-image fit="cover" class="default-img" v-for="(item,index) in movieInfo.moviePictures"
                      :key="index" :src="item" :preview-src-list="movieInfo.moviePictures">
                    </el-image>
                  </div>
                </div>
  ...
  export default {
    ...
    methods: {
      async getMovieInfo() {
        ...
        const {
          data: res
        } = await axios.get('sysMovie/find/' + this.movieId)
        ...
        this.movieInfo.moviePoster = JSON.parse(res.data.moviePoster)[0]
      ...
      //转到播放页面
      toChooseSession() {
        console.log(this.movieInfo.isFreeWatch == 0)
        // console.log(this.loginUser)
        if (this.movieInfo.isFreeWatch == 0) { //判断是否免费电影
          if (this.movieInfo.filmUrl == null) {
            this.$message('还未有播放资源，请谅解');
          } else {
            this.$router.push({
              path: '/watchMovie',
              query: {
                id: this.movieInfo.movieId,
                url: this.movieInfo.filmUrl
              }
            })
            this.recordWatchMovie()
          }
        } else {
          console.log(this.loginUser == '[]')
          //检查是否登录
      ...
      async getServerStatus() {
        ...
        const {
          data: res
        } = await axios.post('/sysUser/getServerStatus/' + id)
        console.info(res)
        if(res.data == true) {
          this.$router.push({
            path: '/watchMovie',
          ...
          this.$message('此电影需要开通会员才能观看');
        }
      },
      //记录观看的电影和用户
      async recordWatchMovie() {
        let watchMovieLog = {
          movieId: this.movieInfo.movieId,
          userId: this.loginUser.userId,
          movieArea: this.movieInfo.movieArea
        }
        console.log(99);
        const {
          data: res
        } = await axios.post('/recordWatchMovie', watchMovieLog)
        console.log(watchMovieLog)
        console.log(res)
      }
