<template>
  <div>
    ...
    <el-carousel :height="carouselHeight">
      <el-carousel-item v-for="item in posterList" :key="item.moviePoster">
        ...
        <div class="panel">
          ...
          <div class="panel-content">
            <div class="board" v-for="(item, index) in totalBoxOfficeList">
              ...
              <div class="board-left">
                <a :href="'/movieInfo/' + item.movieId">
                  <img :src="JSON.parse(item.moviePoster)[0]" style="width: 100px; height: 100px;">
                </a>
              </div>
              <div class="board-middle">
                <a :href="'/movieInfo/' + item.movieId">
                  <p class="name">{{ item.movieName }}</p>
                  <p style="font-size: 2px;  padding: 0px; margin-top: 26px;" class="el-icon-video-play">
                    {{item.watchNum}}</p>
                </a>
  ...
  export default {
    ...
    data() {
      return {
        ...
        posterList: [
          // {url:'http://1.12.42.181:9001/images/20230123_1674474979_302.png' },
    ...
    created() {
      console.log(this.totalBoxOfficeList)
      ...
      this.getLunbotu()
      //赋值目标链接
      // this.movieInfoUrl = '/movieInfo/' + this.ongoingMovieList.movieId
    },
    methods: {
      async getLunbotu() {
        const {
          data: res
        } = await axios.get('sysMovie/getLunbotu')
        console.log(res)
        console.log(22)
      ...
      async getOngoingMovieList() {
        const {
          data: res
        } = await axios.get('sysMovie/findNewestMovie', {
      ...
      async getUpcomingMovieList() {
        const {
          data: res
        } = await axios.get('sysMovie/findVipMovie', {
      ...
      async getClassicMovieList() {
        const {
          data: res
        } = await axios.get('sysMovie/findMovie', {
      ...
      async getTotalBoxOfficeList() {
        const {
          data: resp
        } = await axios.get('sysMovie/find/rankingList/1', {
