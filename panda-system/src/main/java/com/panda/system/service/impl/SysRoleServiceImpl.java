...
public class SysRoleServiceImpl implements SysRoleService {
    ...
    @Override
    public int allotRight(Long rid, Long[] keys) {
        int rows = 0;
        HashSet<Long> originResources = new HashSet<>(sysRoleMapper.findAllRights(rid));

        for (Long id : keys) {
            if (originResources.contains(id)) {
                originResources.remove(id);
            } else {
                rows += sysRoleMapper.addRight(rid, id);
            }
        }
        for (Long id : originResources) {
            rows += sysRoleMapper.deleteRight(rid, id);
        }
        return rows;
    }
}
